...
function usage {
    echo "Usage: $0 -f|--files <FILES> -l|--logfile <LOGFILE> [-d|--datatype <DATATYPE>] [-t|--testtype <TESTTYPE>]"
...
for filename in $FILES*; do
    printf "\n\n === $filename ===\n"
    python3 -u run_propagation.py -f "$filename" -d "$DATATYPE" -t "$TESTTYPE" 2>&1 | tee -a $LOGFILE
done