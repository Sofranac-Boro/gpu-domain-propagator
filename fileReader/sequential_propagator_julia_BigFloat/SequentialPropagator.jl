...
function gdp_retcode_sequential_propagate(n_cons,n_vars,nnz,col_ind,row_ind,vals,
    ...
    csc_vals=Array{BigFloat,1}(zeros(nnz))
...
# test_setup parameters to be defined here?
    ...
    min_acts=Array{BigFloat,1}(zeros(n_cons))
    max_acts=Array{BigFloat,1}(zeros(n_cons))
    min_acts_inf=Array{BigFloat,1}(zeros(n_cons))
    max_acts_inf=Array{BigFloat,1}(zeros(n_cons))
    max_acts_delta=Array{BigFloat,1}(zeros(n_cons))
    # cons_marked=zeros(n_cons)
    # cons_marked_next_round=zeros(n_cons)

    csc_vals,csc_row_ind,csc_col_ptrs = Miscellaneous.csr_to_csc(vals,col_ind,row_ind)
