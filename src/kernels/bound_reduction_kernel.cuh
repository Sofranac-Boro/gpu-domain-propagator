...
//   extern __shared__ datatype shared_mem[]; // contains one array for minacts and another for maxacts
   ...
   if (block_row_end - block_row_begin > 1) // if the block consists of more than 1 row in the matrix
   {
      ...
      if (threadIdx.x < nnz_in_block) {
         ...
         getNewBoundCandidates_no_inf(
                 rhss[considx] - minacts[considx -
                                         block_row_begin], // slack = rhs - minact. minacts:  this is in shared memory - each block's indexing strats from 0, hence the need for - block row begin
                 lhss[considx] - maxacts[considx -
                                         block_row_begin], // surplus = lhs - maxact: maxacts: this is in shared memory - each block's indexing strats from 0, hence the need for - block row begin
                 coeff,
                 lb,
                 ub,
                 &newlb,
      ...
      if (block_row_begin < n_cons) {
         ...
         for (int element = block_data_begin + threadIdx.x; element < block_data_end; element += blockDim.x) {
            ...
            getNewBoundCandidates_no_inf(
                    ...
                    minacts[0], // slack = rhs - minact. minacts: this is in shared memory - each block's indexing strats from 0
                    lhss[block_row_begin] -
                    maxacts[0], // surplus = lhs - maxact: maxacts: this is in shared memory - each block's indexing strats from 0
                    vals[element],
                    lbs[varidx],
                    ubs[varidx],
                    &newlb,
            ...
            FOLLOW_VAR_CALL(
                    ...
                    printf("varidx: %7d, considx: %7d, lhs: %9.2e, rhs: %9.2e, coeff: %9.2e, minact: %9.2e, maxact: %9.2e, oldlb: %9.2e, oldub: %9.2e, newlb: %9.2e, newub: %9.2e\n",
                           ...
                           minacts[0], maxacts[0], lbs[varidx], ubs[varidx], newlb, newub)
            );
...
__global__ void GPUPropEntryKernel
   ...
   for (prop_round = 1; prop_round <= MAX_NUM_ROUNDS && *change_found; prop_round++) {
      *change_found = false;
      ...
      compActsAndBoundsAdaptiveKernel<datatype> <<< blocks_count, NNZ_PER_WG, 2 * max_n_cons_in_block *
                                                                              sizeof(datatype) >>>
              (
                      n_cons, n_vars, max_n_cons_in_block, col_indices, row_ptrs, row_blocks, vals, csc_col_ptrs,
                      csc_row_indices, lbs,
