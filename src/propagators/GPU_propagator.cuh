...
)
{
    ...
    // CUDA_CALL( cudaProfilerStart() );
     ...
     datatype* d_lbs            = gpu.initArrayGPU<datatype> (lbs,             n_vars);
     datatype* d_ubs            = gpu.initArrayGPU<datatype> (ubs,             n_vars);
     datatype* d_newlbs         = gpu.initArrayGPU<datatype> (lbs,             n_vars);
     datatype* d_newubs         = gpu.initArrayGPU<datatype> (ubs,             n_vars);
     int* d_vartypes            = gpu.initArrayGPU<int>      (vartypes,        n_vars);
     ...
     (
         ...
         d_ubs, d_newlbs, d_newubs, d_vartypes, d_lhss, d_rhss, d_change_found
     );
     CUDA_CALL( cudaPeekAtLastError() );
     CUDA_CALL( cudaDeviceSynchronize() );
