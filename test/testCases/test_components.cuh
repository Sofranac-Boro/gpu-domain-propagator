...
TEST_CASE( "test CSR to CSC") {
    ...
    csr_to_csc(gpu, m, n, nnz, d_col_indices, d_row_ptr, csc_col_ptrs.data(), csc_row_indices.data(), csc_vals.data(), d_vals);
    ...
    std::vector<int> exp_row_indices_csc{3, 4, 5, 0, 1, 2, 3, 0, 1, 2, 4, 5}; // dim nnz
     ...
     compareArrays<double>(nnz, exp_vals.data(),         vals.data()            , 1e-6, "vals");
     compareArrays<int>(nnz, exp_col_indices.data(),     col_indices.data()     , 1e-6, "col_indices");
     compareArrays<int>(m+1, exp_row_ptr.data(),         row_ptr.data()         , 1e-6, "row_ptr");
    
     compareCSCRandomRowOrder<double>
     (
         m, n, nnz, csc_vals.data(), csc_row_indices.data(), csc_col_ptrs.data(),
         exp_vals_csc.data(), exp_row_indices_csc.data(), exp_col_ptr_csc.data()
     );
}
...
TEST_CASE( "test CSR to CSC device only") {
    ...
    csr_to_csc_device_only<double>(gpu, m, n, nnz, d_col_indices, d_row_ptr, d_csc_col_ptrs, d_csc_row_indices, d_csr2csc_index_map);

    gpu.getMemFromGPU<int>(d_csc_col_ptrs,      csc_col_ptrs.data(),      n+1);
    gpu.getMemFromGPU<int>(d_csc_row_indices,   csc_row_indices.data(),   nnz);
    gpu.getMemFromGPU<int>(d_csr2csc_index_map, csr2csc_index_map.data(), nnz);
    ...
    compareCSCRandomRowOrder<double>
    (
        m, n, nnz, csr_csc_index_map_double.data(), csc_row_indices.data(), csc_col_ptrs.data(),
        exp_csr2csc_index_map_double.data(), exp_row_indices_csc.data(), exp_col_ptr_csc.data()
    );
}
...
TEST_CASE( "test CSCIndexInverse Kernel")
{
    ...
    compareArrays<int>(nnz, csr2csc_index_map.data(), exp_csr2csc_index_map.data());
}
...
TEST_CASE( "test CSCIndexInverse Kernel big problem")
{
    ...
    for (int i=0; i<nnz; i++)
    {
        csr2csc_index_map.push_back((nnz-1) - i);
        exp_csr2csc_index_map.push_back((nnz-1) - i);
    }
...
TEST_CASE( "test CSR validx to rowidx map kernel test 1") {
    ...
    gpu.getMemFromGPU(d_validx_considx_map, validx_considx_map.data(), nnz);
    compareArrays<int>(nnz, validx_considx_map.data(), exp_validx_considx_map.data() , 1e-6, "exp_validx_considx_map");
}
...
TEST_CASE( "test CSR validx to rowidx map kernel test 2") {
    ...
    {   
        ...
        {
            exp_validx_considx_map.push_back(i);
        }
        row_ptr.push_back(row_ptr_counter);
        row_ptr_counter+=nnz_per_row;
    }
...
TEST_CASE( "initArrayAscending")
{
    ...
    gpu.getMemFromGPU<int>(d_array, array.data(), len);

    std::vector<int> exp_array;
    for (int i=0; i<len; i++)
    {
        exp_array.push_back(i);
    }

    compareArrays<int>(len, array.data(), exp_array.data());
}